const {
  ...
  getBoardListsById,
} = require("../integration/cardIntegration");
const {
...
async function getCardsByListId(id) {
  const cards = await getBoardsFromFileByListId(id);
...
function getBoardsFromFileByListId(id) {
  if (!cards || cards.length === 0) throw new Error("Cards list is empty");

  return cards.filter((card) => card.listId === id);
}

function addCardToFile(card) {
  if (!card) throw new Error("Card data is undefined");
...
async function updateCardInFile(updatedCard) {
  if (!updatedCard) throw new Error("data is undefined");
...
async function deleteCardFromFile(cardId) {
  if (!cardId) throw new Error("id is undefined");

  const updatedCards = deleteElementFromArrayById(cards, cardId);
  writeDataToJsonFile(updatedCards, CARDS_FILE);
}

const getListIdByStatus = (status, lists) =>
  lists.find((list) => list.name === status).id;
